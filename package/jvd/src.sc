...
type VertexEntry = (Long, Json, Json)
  ...
  val (edgeResult, vertexResult) =
    filteredEdges.foldLeft((mutable.Set.empty[EdgeEntry], mutable.Set.empty[VertexEntry])) {
      case ((edgeList, vertexList), edge) =>
        ...
        val inVertexEntry = (edge.inNode.id, Json.fromString(edge.inNode.label), Json.fromValues(edge.inNode.propertyMap.asScala.toList.map { case (key, value) =>
        Json.obj(
          ("key", Json.fromString(key)),
          ("value", Json.fromString(value.toString))
        )
      }))
        val outVertexEntry = (edge.outNode.id, Json.fromString(edge.outNode.label), Json.fromValues(edge.outNode.propertyMap.asScala.toList.map { case (key, value) =>
        Json.obj(
          ("key", Json.fromString(key)),
          ("value", Json.fromString(value.toString))
        )
...
@main def main(prjDir: String, prjName: String): String = {
  importCode(inputPath=prjDir, projectName=prjName)
    ...
    val vertices = vertexEntries.map(x=>Json.fromValues(List(Json.fromLong(x._1), x._2, x._3)))
